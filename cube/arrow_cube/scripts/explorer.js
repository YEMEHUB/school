...
function createCube(){
	...
	var
	// YEME Arrow Cube color layout
	// Net:        black(white class)
	//       orange -> green -> red -> blue
	//             yellow
	// Face order in Cuber maps: [front, up, right, down, left, back]
	// Therefore: front=green, up=black, right=red, down=yellow, left=orange, back=blue.
	W = ERNO.GREEN,   // original front slots become GREEN
	R = ERNO.WHITE,   // original up slots become BLACK via CSS .white
	G = ERNO.RED,     // original right slots become RED
	O = ERNO.YELLOW,  // original down slots become YELLOW
	B = ERNO.ORANGE,  // original left slots become ORANGE
	Y = ERNO.BLUE
	...
	setTimeout( function(){
		...
		for( i = 0; i <= 26; i ++ ){
			...
			cube.cubelets[ i ].faces.forEach( function( face ){
				...
				e.style.translate = '0 10px';

				if(
				   face.color.name === 'red' ||
  				   face.color.name === 'green' // 검정으로 바꿀 면 		
				   ){
			    e.style.transform = 'rotate(-90deg)';
				}
                if(
				   face.color.name === 'white' // 검정으로 바꿀 면
 	     		){
			    e.style.transform = 'rotate(90deg)';
				}
		...
		// YEME: logo removed for picture-cube stickers
		// cube.showLogo()


		// YEME Arrow Picture Cube Patch v5
		// 숫자 라벨의 방향 추적 기능을 그대로 사용합니다.
		// 숫자+밑줄을 '아래 방향 화살표 SVG'로만 교체합니다.
		var arrowSvg = '<svg class="cubeArrow" viewBox="0 0 100 100" aria-hidden="true">' +
			'<path d="M40 10 H60 V58 H78 L50 90 L22 58 H40 Z"></path>' +
			'</svg>'

		cube.cubelets.forEach( function( cubelet ){
			cubelet.faces.forEach( function( face ){
				var id = face.element.querySelector( '.id' )
				var underline = face.element.querySelector( '.underline' )
				var sticker = face.element.querySelector( '.sticker' )
				if( sticker && id && underline ){
					underline.innerHTML = arrowSvg
	...
	// YEME: startup demo removed. Show cube immediately for classroom use.
...
function createControls(){
	...
	window.isDemoing = false// YEME: no startup demo
